...
export function createCamera(gameWindow) {
    ...
    function onMouseMove(event) {
        ...
        if (isMiddleMouseDown) {
            const forward = new THREE.Vector3(0, 0, 1).applyAxisAngle(Y_AXIS, cameraAzimuth * DEGREE_TO_RAD);
            const left = new THREE.Vector3(1, 0, 0).applyAxisAngle(Y_AXIS, cameraAzimuth * DEGREE_TO_RAD);
    ...
    function updateCameraPosition() {
        camera.position.x = cameraRadius * Math.sin(cameraAzimuth * DEGREE_TO_RAD) * Math.cos(cameraElevation * DEGREE_TO_RAD);
        camera.position.y = cameraRadius * Math.sin(cameraElevation * DEGREE_TO_RAD);
        camera.position.z = cameraRadius * Math.cos(cameraAzimuth * DEGREE_TO_RAD) * Math.cos(cameraElevation * DEGREE_TO_RAD);
